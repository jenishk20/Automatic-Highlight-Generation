...
DEFAULT_CONFIG = {
    ...
    'use_wandb': False,  # Set to True to use Weights & Biases for tracking experiments
    'wandb_project': 'football-key-moment-classification',
    ...
    'scheduler': 'cosine',  # 'cosine', 'reduce_on_plateau', 'none'
    ...
    # Model parameters
    'model_type': 'slow_fast',  # 'r3d', 'mc3', 'r2plus1d', 'i3d', 'slow_fast', 'x3d'
    'dropout': 0.5,
    'pretrained': True,
    ...
    # Cross-validation
    'num_folds': 5,
    'fold_type': 'stratified',  # 'stratified' or 'standard'
...
class SlowFastNetwork(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        ...
        self.slowfast = video_models.slowfast_r50(pretrained=pretrained)
        
        # Replace the final fully connected layer
        # SlowFast has two pathways, so we need to modify the head
        in_features = self.slowfast.head.fc.in_features
        self.slowfast.head.fc = nn.Linear(in_features, num_classes)
...
class I3DModel(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        ...
        self.i3d = video_models.i3d_resnet50(pretrained=pretrained)
        
        # Replace the final fully connected layer
        in_features = self.i3d.fc.in_features
        self.i3d.fc = nn.Linear(in_features, num_classes)
...
class X3DModel(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        ...
        self.x3d = video_models.x3d_m(pretrained=pretrained)
        
        # Replace the final fully connected layer
        in_features = self.x3d.fc.in_features
        self.x3d.fc = nn.Linear(in_features, num_classes)
...
class R2Plus1DModel(nn.Module):
    def __init__(self, num_classes, dropout=0.5, pretrained=True):
        ...
        self.r2plus1d = video_models.r2plus1d_18(pretrained=pretrained)
...
def create_model(config, num_classes):
    ...
    if model_type == 'r3d':
        ...
        return R2Plus1DModel(num_classes, dropout, pretrained)
    elif model_type == 'i3d':
        return I3DModel(num_classes, pretrained)
    elif model_type == 'slow_fast':
        return SlowFastNetwork(num_classes, pretrained)
    elif model_type == 'x3d':
        return X3DModel(num_classes, pretrained)
    else:
...
def train_with_cross_validation(config):
    ...
    if config['fold_type'] == 'stratified':
        ...
        splits = kfold.split(video_paths)
    ...
    for fold, (train_idx, val_idx) in enumerate(splits):
        ...
        for epoch in range(config['num_epochs']):
            ...
            if config['use_wandb']:
                wandb.log({
                    f"fold_{fold+1}/train_loss": epoch_train_loss,
                    f"fold_{fold+1}/train_acc": epoch_train_acc,
                    f"fold_{fold+1}/val_loss": epoch_val_loss,
                    f"fold_{fold+1}/val_acc": epoch_val_acc,
...
def perform_ablation_study(base_config):
    ...
    ablation_configs = [
        ...
        {'name': 'r2plus1d_18', 'model_type': 'r2plus1d', 'batch_size': 8},
        {'name': 'i3d', 'model_type': 'i3d', 'batch_size': 8},
        {'name': 'slowfast', 'model_type': 'slow_fast', 'batch_size': 4},  # SlowFast needs smaller batch size
        {'name': 'x3d', 'model_type': 'x3d', 'batch_size': 8},
        ...
        {'name': 'adam', 'optimizer': 'adam', 'model_type': 'r3d'},
        ...
        {'name': 'frames_64', 'num_frames': 64, 'model_type': 'r3d'},
        
        # Different frame sizes
        {'name': 'size_112', 'frame_height': 112, 'frame_width': 112, 'model_type': 'r3d'},
    ...
    for config_mods in ablation_configs:
        ...
        config['num_folds'] = 3  # Use fewer folds for speed
